...
import { Users, Search, Download, CheckCircle2, Bot, Filter, Smartphone, Globe, RefreshCw } from 'lucide-react';
...
export default function BeneficiariosSocialPage() {
  ...
  const fetchBeneficiaries = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/social/beneficiaries', {
        headers: {
      ...
      console.warn('Error fetching beneficiaries, using initial data:', e);
    }

    // Default fallback demo data if DB is cold
    setBeneficiaries([
      {
        id: 101,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Registro consolidado de postulaciones capturadas por agentes IA, WhatsApp y formularios de programas sociales
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Card className="p-4 bg-emerald-500/10 border-emerald-500/20">
          ...
          <p className="text-2xl font-bold text-foreground mt-1">
            {beneficiaries.filter(b => b.intake_channel?.includes('whatsapp') || b.intake_channel?.includes('agent') || b.intake_channel?.includes('conversational')).length}
          </p>
        </Card>
              ...
              {filtered.map((b) => (
                <tr key={b.id} className="hover:bg-muted/30 transition-colors">
                  <td className="py-3 px-4">
                    <p className="font-semibold text-foreground leading-snug">{b.full_name}</p>
