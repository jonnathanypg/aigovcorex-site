  ...
  try {
    ...
    if (audioMsg) {
      attachment = {
        ...
        filename: `wa_audio_${Date.now()}.${audioMsg.mimetype?.split('/')[1]?.split(';')[0] || 'ogg'}`,
      };
      console.log(`[${data.companyId}] Audio message detected: ${attachment.filename}`);
      ...
      try {
        const stream = await Baileys.downloadMediaMessage(
          ...
          {},
          // @ts-ignore
          { logger: pino({ level: "silent" }), reuploadRequest: (wsTransporter as any).sessions?.get(data.companyId)?.socket?.updateMediaMessage }
        );
        if (stream) {
          const fs = await import("fs");
          const path = await import("path");
          ...
          const filePath = path.default.join(tmpDir, attachment.filename);
          fs.default.writeFileSync(filePath, stream as Buffer);
          attachment.local_path = filePath;
          console.log(`[${data.companyId}] Audio saved to: ${filePath}`);
        }
      } catch (dlErr: any) {
        console.error(`[${data.companyId}] Failed to download audio: ${dlErr.message}`);
      }
    }
